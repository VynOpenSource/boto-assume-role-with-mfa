...
class SessionCacheTest(unittest.TestCase):
    ...
    def test_cache_manager_warm(self):
        ...
        one_hour_in_future = (now + datetime.timedelta(hours=1)).strftime(
            "%Y-%m-%dT%H:%M:%S%Z"
        )
...
class CachedMfaSessionFactoryTest(unittest.TestCase):
    ...
    def test_temporary_credentials_in_cache(self):
        ...
        cache.__setitem__(
            CachedMfaSessionFactory.SESSION_KEY, get_current_session_data()
        )

        cached_mfa_session_factory = CachedMfaSessionFactory(
            sts_client=Mock(BaseClient), session_cache=SessionCache(cache=cache)
        )
        ...
        self.assertEqual(
            credentials.get("SecretAccessKey"), "temporary_secret_access_key"
        )
    ...
    def test_temporary_credentials_from_session(self):
        ...
        cached_mfa_session_factory = CachedMfaSessionFactory(
            sts_client=sts_client, session_cache=SessionCache(cache=cache)
        )
        ...
        self.assertEqual(
            temporary_session_data.get("AccessKeyId"), "temporary_access_key_id"
        )
        self.assertEqual(
            temporary_session_data.get("SecretAccessKey"), "temporary_secret_access_key"
        )
